...
class RiskAverseQLearning:
    ...
    def update_state(self):
        ...
        self.cur_state = tuple(state)
        print("currrent state: ", self.cur_state)
    ...
    def get_action_tuple(self, Q_hat_index):
        ...
        if self.cur_state is None:
            self.init_state()
        if self.cur_state not in Q_hat_:
            return self.get_random_action_tuple()
        Q_hat_state = Q_hat_[self.cur_state]
        curMaxNegAction = [None, -1e9]
        curMaxPosAction = [None, -1e9]
        negActionCount = 0
        for action in itertools.product(range(3), repeat=self.num_devices):
            if action not in Q_hat_state:
                continue
            v = Q_hat_state[action]
            if v < 0:
        ...
        if curMaxPosAction[0] is not None:
            return curMaxPosAction[0]
        if negActionCount == 3**self.num_devices:
            return curMaxNegAction[0]
        aList = []
        for action in itertools.product(range(3), repeat=self.num_devices):
            if action not in Q_hat_state:
                aList.append(action)
    ...
    def update_to_new_state(self, reward, cur_frame, action, sample_achievable_rate):
        ...
        for i, v in enumerate(msk):
            ...
            # find max Action in current Q
            max_QA = 0
            if new_state in self.Q_table[i]:
                Q_tab = self.Q_table[i][new_state]
                maxPos = -1e9
                negCount = 0
                maxNeg = -1e9
                for i_action in itertools.product(range(3), repeat=self.num_devices):
                    if i_action not in Q_tab:
                        continue
                    else:
                        v = Q_tab[i_action]
                        if v > 0 and v > maxPos:
        ...
        for i in range(self.num_Qtable):
            ...
            sanity_check = self.Q_table[i][old_state][action]
            qavg = sum(self.Q_table[a][old_state][action] for a in range(self.num_Qtable)) / self.num_Qtable
            
            sanity_check -= self.risk_control / (self.num_Qtable-1) * sum((self.Q_table[a][old_state][action]-qavg)**2 for a in range(self.num_Qtable))
